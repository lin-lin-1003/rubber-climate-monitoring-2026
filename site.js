...
function zipSafeName(value) { return String(value || "未命名").replace(/[\\/:*?"<>|]/g, "-").trim() || "未命名"; }
function sha256Hex(value) { return crypto.subtle.digest("SHA-256", new TextEncoder().encode(value)).then(buffer => [...new Uint8Array(buffer)].map(byte => byte.toString(16).padStart(2, "0")).join("")); }
const crcTable = (() => { const table = new Uint32Array(256); for (let index = 0; index < 256; index += 1) { let code = index; for (let bit = 0; bit < 8; bit += 1) code = (code & 1) ? (0xedb88320 ^ (code >>> 1)) : (code >>> 1); table[index] = code >>> 0; } return table; })();
...
async function exportChartPng(station, metric, host) {
  ...
  copy.insertAdjacentHTML("afterbegin", '<style>.chart-grid{stroke:#e3ebe9;stroke-width:1}.chart-label{font-family:Arial,"PingFang SC","Microsoft YaHei",sans-serif;font-size:10px;fill:#62767b}.export-title{font-family:Arial,"PingFang SC","Microsoft YaHei",sans-serif;font-size:16px;font-weight:700;fill:#173036}</style><rect width="660" height="300" fill="#fbfdfc"/><text x="18" y="20" class="export-title">' + esc(metric.label) + '（' + esc(metric.unit) + '）</text><rect x="18" y="28" width="18" height="8" fill="#d9eaf4"/><text x="40" y="35" class="chart-label">历史最低—最高</text><line x1="128" x2="146" y1="32" y2="32" stroke="#d92d20" stroke-width="3.5"/><text x="150" y="35" class="chart-label">本年实况</text><line x1="225" x2="243" y1="32" y2="32" stroke="#c66628" stroke-width="2" stroke-dasharray="5 3"/><text x="247" y="35" class="chart-label">未来预测</text>' + selectedLegend);
...
async function exportAnnualImages() {
  ...
  try {
    ...
    for (let index = 0; index < tasks.length; index += 1) {
      const {station, metric} = tasks[index];
      try {
        const data = await exportChartPng(station, metric, host);
        ...
        console.error(error); failures.push(`${station.country}·${station.region}·${station.station_name}（${metric.label}）`);
      }
      button.textContent = `正在绘制网页年度图 ${index + 1}/${tasks.length} · 已跳过 ${failures.length}`;
    }
    if (!entries.length) throw new Error("所有地点图均导出失败");
    ...
    alert(failures.length ? `导出完成：已跳过 ${failures.length} 张图片。\\n${failures.join("\\n")}` : `导出完成：共 ${entries.length} 张图片。`);
...
function renderControls() {
  ...
  byId("export-images").addEventListener("click", exportAnnualImages);
...
function drawCard(card) {
  ...
  body += '<path class="actual-line" d="' + pathSegments(series.actual,x,y) + '" fill="none" stroke="#d92d20" stroke-width="3.5"/>' +
    '<path d="' + pathSegments(series.forecast,x,y) + '" fill="none" stroke="#c66628" stroke-width="2" stroke-dasharray="5 3"/>' +
