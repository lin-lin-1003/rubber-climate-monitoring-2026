...
function renderControls() {
  ...
  byId("annual-controls").innerHTML = '<div class="metric-checks" role="group" aria-label="显示指标">' +
    ...
    allRegions().filter(r => r.has_anomaly || r.has_missing).map(r => '<label><input type="checkbox" data-region="' + esc(r.region_key) + '"' + (state.selected.has(r.region_key) ? ' checked' : '') + '><span>' + esc(r.country + " · " + r.production_region) + '<small>' + esc(r.status) + ' · 影响分 ' + fmt(r.impact_score, 2) + (r.has_missing ? ' · 含数据不足项' : '') + '</small></span></label>').join("") +
